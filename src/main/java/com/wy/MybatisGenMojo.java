...
import com.wy.strategy.NameStrategy;
import com.wy.strategy.NameStrategyFactory;
...
public class MybatisGenMojo extends AbstractMojo {
    ...
    private Clazz getClazz(NameStrategy nameStrategy, Connection conn, String table) throws MojoExecutionException, SQLException {
        ...
        while (resultSet.next()) {
            ...
            String javaType = TypeMap.getJdbcTypeToJavaMap().get(jdbcType);
...
//        clazz.setClassName(tableMap.get(table));
    ...
    /**
     * 构建数据对象名称，当自定义的表名不存在的时候，用
     * @param tableName 表名
     * @return 数据对象名称
     */
    private String getEntityName(String tableName){
        // 如果自定义的表名为空，则根据表名生成实体类名
        final String entityName = tableMap.get(tableName);
        if (StringUtils.isNotEmpty(entityName)) {
            return entityName;
        }
        StringBuilder result = new StringBuilder();
        // 排除表名的前缀
        String fixedTableName = StringUtils.stripStart(tableName, "t_");
        String[] nameSections = fixedTableName.split("_");
        for (String section : nameSections) {
            String camelSection = section.substring(0, 1).toUpperCase();
            result.append(camelSection);
